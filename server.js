...
// ⚠️ Kalau ini jalan di Railway (atau platform serupa) TANPA volume permanen di-attach,
// database & foto akan HILANG TOTAL tiap kali redeploy — filesystem container itu sekali pakai.
// Railway set env var RAILWAY_ENVIRONMENT otomatis; kita pakai itu buat deteksi & ngingetin.
if (process.env.RAILWAY_ENVIRONMENT && (!process.env.DB_PATH || !process.env.UPLOADS_DIR)) {
  console.warn('');
  console.warn('⚠️⚠️⚠️  PERINGATAN PENTING  ⚠️⚠️⚠️');
  console.warn('Terdeteksi jalan di Railway, tapi env var DB_PATH/UPLOADS_DIR belum di-set.');
  console.warn('Tanpa Volume permanen, SEMUA data (pesanan, menu, foto, akun) akan HILANG');
  console.warn('setiap kali kamu redeploy / restart service ini.');
  console.warn('Perbaiki: buka Railway dashboard → service ini → tab "Volumes" → Add Volume,');
  console.warn('mount ke "/app/data", lalu set env var DB_PATH=/app/data/nokayune.db,');
  console.warn('UPLOADS_DIR=/app/data/uploads, dan BACKUPS_DIR=/app/data/backups.');
  console.warn('Detail lengkap ada di README.md.');
...
app.use('/api/customers/register', customerAuthLimiter);
...
function backupDatabase() {
  const src = db.DB_PATH;
  if (!fs.existsSync(src)) return;
  const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  const dest = path.join(BACKUPS_DIR, `nokayune-${stamp}.db`);
  try {
    // Use SQLite backup API for consistent backup
    db.backup(dest).then(() => {
      console.log(`📦 Database di-backup ke ${BACKUPS_DIR}`);
    ...
    try {
      ...
      console.log(`📦 Database di-backup (fallback) ke ${BACKUPS_DIR}`);
    } catch (e2) {
