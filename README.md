...
- **Stok menu** — tiap menu bisa diisi jumlah stok (kosongkan kalau tak terbatas). Stok
  otomatis berkurang tiap ada pesanan masuk, dan status berubah "Habis" sendiri begitu stok
  mencapai 0. Ada juga peringatan "Stok Menipis" di dashboard kalau tersisa ≤5.
- **Multi-wilayah** — cocok kalau usahamu sudah/mau buka di lebih dari satu kota. Dua wilayah
  contoh (Purwokerto & Jember) sudah disiapkan, dan kamu bisa menambah wilayah baru sendiri
  lewat Admin → Pengaturan → Kelola Wilayah, masing-masing dengan nomor WhatsApp dan alamat
  sendiri. Pelanggan memilih wilayahnya lewat ikon 📍 di navbar, dan menu/stok bisa diatur
  khusus per wilayah (atau dibiarkan tersedia untuk semua wilayah).
...
nokayune-backend/
  ...
  routes/            ← menu, orders, admin, stats, payment, regions (wilayah)
