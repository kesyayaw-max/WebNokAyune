...
> ⚠️ **WAJIB dibaca sebelum deploy:** Railway (dan Docker pada umumnya) memakai filesystem
> yang **sekali pakai** — kalau kamu redeploy atau restart service tanpa Volume permanen,
> SEMUA data (pesanan, menu, foto, akun admin/pelanggan) akan **hilang total** dan balik ke
> kondisi awal. Ini bukan bug, tapi cara kerja platform-nya. Langkah "Setup Volume" di bawah
> ini WAJIB, bukan opsional, kalau websitenya beneran dipakai untuk usaha.

### Langkah 0 — Setup Volume permanen (WAJIB, sebelum deploy pertama kali)
1. Di Railway dashboard, buka service kamu → tab **Volumes** → **+ New Volume**
2. Mount path: `/app/data`
3. Tambahkan environment variable berikut (di tab **Variables**):
   ```
   DB_PATH=/app/data/nokayune.db
   UPLOADS_DIR=/app/data/uploads
   BACKUPS_DIR=/app/data/backups
   ```
4. Kalau kamu skip langkah ini, server akan tetap jalan (dan bahkan mengingatkan lewat log),
   tapi datanya tidak aman — foto & pesanan bisa hilang kapan saja tanpa peringatan ke pelanggan.
...
4. Tambahkan environment variable:
   ...
   ```
   (plus 3 variable `DB_PATH`/`UPLOADS_DIR`/`BACKUPS_DIR` dari Langkah 0 di atas)
...
3. Set env variable `JWT_SECRET` (+ variable dari Langkah 0 kalau deploy manual di server sendiri;
   kalau pakai `docker-compose.yml` yang disediakan, variable ini sudah otomatis diatur)
4. Deploy

### Deploy di server sendiri / VPS dengan Docker Compose
`docker-compose.yml` yang disediakan sudah otomatis mount folder `./data` di host ke
`/app/data` di container — jadi database, foto, dan backup semuanya aman tersimpan di luar
container. Tinggal jalankan:
```bash
