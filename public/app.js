...
async function loadMenu() {
  try {
    menuData = await apiFetch('/api/menu?status=available');
  } catch (e) {
...
function menuCard(m) {
  ...
  return `<div class="menu-card">
    <div class="menu-card-img">${imgInner}${soldOut ? '<span class="menu-card-badge soldout">Habis</span>' : ''}</div>
...
async function goToPayment() {
  ...
  try {
    const order = await apiFetch('/api/orders', {
      method: 'POST',
      body: JSON.stringify({ name, phone, address, time, notes, items: cart.map(c => ({ id: c.id, qty: c.qty })) }),
    });
    currentOrder = order;
...
function openWA() {
  ...
  const waNumber = (storeSettings.owner_wa || '6281234567890').replace(/\D/g, '');
...
async function renderAdminOrders(filter, page) {
  ...
  try {
    const res = await apiFetch(`/api/orders?status=${ordersFilter}&page=${ordersPage}&limit=10`);
...
function orderCard(o) {
  ...
  return `<div class="order-card">
    ...
    <div class="order-details">${o.address}<br/>🍽️ ${itemsText}<br/>💳 ${o.payment_status === 'paid' ? 'Sudah dibayar' : 'Belum dibayar'}</div>
...
function showAddMenu() {
  ...
  ['mf-name', 'mf-price', 'mf-emoji', 'mf-desc'].forEach(id => document.getElementById(id).value = '');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadMenu();
  apiFetch('/api/admin/settings').then(s => { storeSettings = s; }).catch(() => {});
  updateCartCount();
