...
// Semua teks yang datang dari input orang (nama, alamat, catatan pesanan, komentar review, dsb)
// WAJIB lewat fungsi ini sebelum ditaruh ke innerHTML — kalau tidak, orang bisa nyelipin kode
// script lewat kolom-kolom itu (XSS) dan kejalanin di browser pengunjung lain / admin.
    ...
    if (!orders.length) {
      ...
      grid.innerHTML = orders.map(o => `
        <div class="kitchen-card">
          ...
          <div class="kit-customer">${escapeHtml(o.name)}</div>
          <div class="kit-address">📍 ${escapeHtml(o.address)}</div>
          <div class="kit-items">${o.items.map(it => `<div class="kit-item"><span>${escapeHtml(it.name)} ×${it.qty}</span><span>${fmtRp(it.price*it.qty)}</span></div>`).join('')}</div>
...
let customerAuthMode = 'login'; // 'login' or 'register'

function toggleCustomerAuthMode() {
  customerAuthMode = customerAuthMode === 'login' ? 'register' : 'login';
  const nameGroup = document.getElementById('custNameGroup');
  const title = document.getElementById('custLoginTitle');
  const submitBtn = document.getElementById('custSubmitBtn');
  const toggleText = document.getElementById('custToggleText');
  document.getElementById('custError').textContent = '';
  if (customerAuthMode === 'register') {
    nameGroup.style.display = '';
    title.textContent = 'Daftar Akun Baru';
    submitBtn.textContent = 'Daftar';
    toggleText.innerHTML = 'Sudah punya akun? <a href="#" onclick="toggleCustomerAuthMode(); return false;">Masuk di sini</a>';
  } else {
    nameGroup.style.display = 'none';
    title.textContent = 'Masuk';
    submitBtn.textContent = 'Masuk';
    toggleText.innerHTML = 'Belum punya akun? <a href="#" onclick="toggleCustomerAuthMode(); return false;">Daftar di sini</a>';
  }
}
...
async function customerLogin() {
  ...
  if (!phone || !pin) { errEl.textContent = 'Lengkapi nomor WhatsApp dan PIN.'; return; }
  if (pin.length !== 4) { errEl.textContent = 'PIN harus 4 digit.'; return; }
  if (customerAuthMode === 'register' && !name) { errEl.textContent = 'Nama lengkap wajib diisi.'; return; }

  try {
    // Mode login dan daftar sekarang eksplisit dipilih sendiri oleh orangnya (bukan ditebak
    // sistem) — jadi kalau PIN salah pas login, pesannya jelas "PIN salah", bukan malah nyasar
    // ke pesan "nomor sudah terdaftar" gara-gara backend diam-diam nyoba daftarin ulang.
    const endpoint = customerAuthMode === 'register' ? '/api/customers/register' : '/api/customers/login';
    const body = customerAuthMode === 'register' ? { name, phone, pin } : { phone, pin };
...
async function loadMyOrders() {
  ...
  try {
    ...
    el.innerHTML = orders.map(o => {
      const itemsText = o.items.map(it => `${escapeHtml(it.name)} ×${it.qty}`).join(', ');
      return `<div class="my-order-card">
        ...
        <div class="my-order-items">🍽️ ${itemsText}${o.region_name ? ` · 📍 ${escapeHtml(o.region_name)}` : ''}</div>
  ...
  try {
    ...
    grid.innerHTML = data.reviews.map((r, i) => `
      <div class="testi-card ${i === 0 ? 'featured-testi' : ''}">
        ...
        <p>"${escapeHtml(r.comment) || 'Makanannya enak!'}"</p>
        <div class="testi-author"><span class="testi-avatar">${escapeHtml((r.name || 'P')[0].toUpperCase())}</span><div><strong>${escapeHtml(r.name) || 'Pelanggan'}</strong><small>${new Date(r.created_at).toLocaleDateString('id-ID', {month:'long',year:'numeric'})}</small></div></div>
      </div>`).join('');
...
async function loadRegions() {
  ...
  if (sel) {
    sel.innerHTML = regions.map(r => `<option value="${r.id}">${escapeHtml(r.name)}</option>`).join('') || '<option>Belum ada wilayah</option>';
...
function showPage(name) {
  ...
  if (name === 'customer-login') {
    ...
    const toggleText = document.getElementById('custToggleText');
    if (toggleText) toggleText.innerHTML = 'Belum punya akun? <a href="#" onclick="toggleCustomerAuthMode(); return false;">Daftar di sini</a>';
    const errEl = document.getElementById('custError');
    if (errEl) errEl.textContent = '';
    ['cust-name', 'cust-phone', 'cust-pin'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  }
  updateCartCount();
  updateFloatWA();
}
...
async function confirmPayment() {
  try {
    const order = await apiFetch('/api/payment/confirm-manual/' + currentOrder.id, {
      method: 'POST',
      body: JSON.stringify({ phone: currentOrder.phone }),
    });
...
function renderSuccess(order) {
  ...
  el.innerHTML = `
    ...
    <p>👤 <strong>Pemesan:</strong> ${escapeHtml(order.name)}</p>
...
async function trackOrder() {
  ...
  try {
    ...
    const items = order.items.map(it => `<div class="checkout-item"><span>${escapeHtml(it.name)} ×${it.qty}</span><span>${fmtRp(it.price * it.qty)}</span></div>`).join('');
...
async function loadAdminRegions() {
  ...
  const fillSelect = (el, includeAll, includeUnset) => {
    ...
    opts += adminRegions.map(r => `<option value="${r.id}">${escapeHtml(r.name)}${r.active ? '' : ' (nonaktif)'}</option>`).join('');
    el.innerHTML = opts;
...
function renderRegionsList() {
  ...
  el.innerHTML = adminRegions.map(r => `
    <div class="top-menu-item">
      <span>📍 ${escapeHtml(r.name)}${r.active ? '' : ' <em style="color:var(--gray);">(nonaktif)</em>'}<br/><small style="font-weight:400;color:var(--gray);">${r.wa_number ? '📱 ' + escapeHtml(r.wa_number) : ''} ${r.address ? '· ' + escapeHtml(r.address) : ''}</small></span>
...
function adminTab(tab, el) {
  ...
  if (tab === 'messages') loadContactMessages();
}
...
function orderCard(o) {
  ...
  return `<div class="order-card">
    ...
    <div class="order-customer">${escapeHtml(o.name)} — ${escapeHtml(o.phone)}</div>
    <div class="order-details">${escapeHtml(o.address)}${o.region_name ? ` · 📍 ${escapeHtml(o.region_name)}` : ''}<br/>🍽️ ${itemsText}</div>
...
async function loadContactMessages() {
  ...
  try {
    ...
    if (!messages.length) {
      el.innerHTML = `<div class="empty-state"><div class="empty-icon">✉️</div><h3>Belum ada pesan</h3><p>Pesan dari form "Hubungi Kami" akan muncul di sini.</p></div>`;
      return;
    }
    el.innerHTML = messages.map(m => `
      <div class="order-card" style="${m.read ? 'opacity:0.65;' : ''}" onclick="markMessageRead(${m.id})">
        <div class="order-header">
          <span class="order-id">${escapeHtml(m.name)} · ${escapeHtml(m.phone)}</span>
          <span>${m.read ? '' : '<span class="pay-unpaid">Baru</span>'}</span>
        </div>
        <div class="order-details">${escapeHtml(m.message)}</div>
        <div style="font-size:0.78rem;color:var(--gray);margin-top:6px;">${new Date(m.created_at).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</div>
      </div>`).join('');
  } catch (e) {
    el.innerHTML = `<p style="text-align:center;color:var(--clay);padding:24px;">${e.message}</p>`;
  }
}
...
async function loadCustomersList() {
  ...
  if (!el) return;
  el.innerHTML = '<p style="color:var(--gray);font-size:0.85rem;">Memuat...</p>';
  try {
    const customers = await apiFetch('/api/customers');
    if (!customers.length) {
      el.innerHTML = '<p style="color:var(--gray);font-size:0.85rem;">Belum ada pelanggan yang daftar akun.</p>';
      return;
    }
    el.innerHTML = customers.map(c => `
      <div class="top-menu-item">
        <span>${escapeHtml(c.name)} — 📱 ${escapeHtml(c.phone)}</span>
        <button class="edit-btn" style="flex:none;padding:4px 10px;" onclick="resetCustomerPin(${c.id}, '${escapeHtml(c.name)}')">🔑 Reset PIN</button>
      </div>`).join('');
  } catch (e) {
    el.innerHTML = `<p style="color:var(--clay);font-size:0.85rem;">${e.message}</p>`;
  }
}

async function resetCustomerPin(id, name) {
  if (!await showConfirm('Reset PIN Pelanggan', `PIN untuk ${name} akan diganti PIN baru secara acak. Lanjutkan?`)) return;
  try {
    const res = await apiFetch(`/api/customers/${id}/reset-pin`, { method: 'POST' });
    await showConfirm('PIN Berhasil Direset', `PIN baru untuk ${res.name} (${res.phone}) adalah ${res.newPin}. Sampaikan PIN ini ke pelanggan lewat WhatsApp.`);
  } catch (e) {
    showToast(e.message, 'error');
  }
}
