...
module.exports = function ordersRouter(io) {
  ...
  router.post('/', (req, res) => {
    ...
    // Validate phone format (Indonesia: 08xx, 10-14 digits total)
    if (phone && !/^(\+62|62|0)8[1-9][0-9]{6,12}$/.test(phone.replace(/[\s\-()]/g, ''))) {
      return res.status(400).json({ error: 'Format nomor WhatsApp tidak valid. Gunakan format 08xxxxxxxxxx.' });
    }
    // Check cutoff time
    const cutoffRow = db.prepare("SELECT value FROM settings WHERE key = 'cutoff_time'").get();
    const cutoff = cutoffRow?.value || '20:00';
    const now = new Date();
    const [cutH, cutM] = cutoff.split(':').map(Number);
    const cutoffToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), cutH, cutM, 0);
    if (now >= cutoffToday) {
      return res.status(400).json({ error: `Maaf, pemesanan sudah ditutup untuk hari ini (jam ${cutoff}). Silakan pesan lagi besok sebelum jam ${cutoff}.` });
    }
    // Validate deliver time is in the future
    if (time && new Date(time) <= new Date()) {
      return res.status(400).json({ error: 'Waktu pengiriman harus di masa depan.' });
    }
    ...
    // ID lama cuma 6 digit terakhir dari timestamp (gampang ditebak/diulang). Tambah bagian
    // acak biar jauh lebih sulit ditebak orang lain, dan tetap dicek keunikannya ke DB.
    function generateOrderId() {
      const rand = Math.random().toString(36).slice(2, 6).toUpperCase();
      return 'NA' + Date.now().toString().slice(-6) + rand;
    }
    let orderId = generateOrderId();
    while (db.prepare('SELECT 1 FROM orders WHERE id = ?').get(orderId)) orderId = generateOrderId();
  ...
  router.patch('/:id/status', requireAuth, (req, res) => {
    ...
    const valid = ['pending', 'confirmed', 'cooking', 'delivered', 'done', 'cancelled'];
  ...
  router.patch('/:id/cancel', requireAuth, (req, res) => {
    ...
    if (order.status === 'cancelled' || order.status === 'done' || order.status === 'delivered') {
      return res.status(400).json({ error: 'Pesanan tidak bisa dibatalkan karena sudah selesai atau sudah dibatalkan.' });
    }
    const tx = db.transaction(() => {
      // Restore stock
      const items = db.prepare('SELECT menu_id, qty FROM order_items WHERE order_id = ?').all(order.id);
      const restoreStock = db.prepare(`UPDATE menu SET stock = stock + ?, status = CASE WHEN stock + ? > 0 THEN 'available' ELSE status END WHERE id = ? AND stock IS NOT NULL`);
      const unBump = db.prepare('UPDATE menu SET sold = MAX(0, sold - ?) WHERE id = ?');
      for (const it of items) {
        restoreStock.run(it.qty, it.qty, it.menu_id);
        unBump.run(it.qty, it.menu_id);
      }
