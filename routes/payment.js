...
module.exports = function paymentRouter(io) {
  ...
  //
  // IMPORTANT: this requires the order's phone number too (not just the ID). Order IDs alone
  // aren't secret enough to trust — requiring the phone the customer used at checkout means an
  // outsider can't mark a stranger's order "paid" just by guessing/observing an order ID.
  router.post('/confirm-manual/:orderId', (req, res) => {
    const { phone } = req.body || {};
    if (!phone) return res.status(400).json({ error: 'Nomor WhatsApp pemesan wajib disertakan.' });
    const order = db.prepare('SELECT * FROM orders WHERE id = ?').get(req.params.orderId);
    if (!order || order.phone !== phone) {
      return res.status(404).json({ error: 'Pesanan tidak ditemukan. Periksa kembali nomor pesanan & WhatsApp.' });
    }
    db.prepare("UPDATE orders SET payment_status = 'paid', status = CASE WHEN status='pending' THEN 'confirmed' ELSE status END WHERE id = ?")
      .run(order.id);
