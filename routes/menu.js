...
router.post('/', requireAuth, requireRole('owner'), upload.single('photo'), (req, res) => {
  const { name, cat, price, emoji, desc, status } = req.body;
  if (!name || !cat || !price) return res.status(400).json({ error: 'Nama, kategori, dan harga wajib diisi.' });
  const image = req.file ? `/uploads/${req.file.filename}` : null;
  const info = db.prepare(`INSERT INTO menu (name, cat, price, emoji, image, desc, status, sold) VALUES (?,?,?,?,?,?,?,0)`)
    .run(name, cat, Number(price), emoji || '🍽️', image, desc || '', status || 'available');
...
router.put('/:id', requireAuth, requireRole('owner'), upload.single('photo'), (req, res) => {
  ...
  db.prepare(`UPDATE menu SET name=?, cat=?, price=?, emoji=?, image=?, desc=?, status=? WHERE id=?`)
    .run(name ?? existing.name, cat ?? existing.cat, price ? Number(price) : existing.price,
         emoji ?? existing.emoji, image, desc ?? existing.desc, status ?? existing.status, req.params.id);
